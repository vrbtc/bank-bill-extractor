#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""
重新检查建设银行、民生银行、交通银行的账单金额
重点区分：本期账单金额 vs 最低还款额
"""

import imaplib
import email
from email.header import decode_header
import re


EMAIL_ADDRESS = "rrking@aliyun.com"
PASSWORD = "Aa2599589"
IMAP_SERVER = "imap.aliyun.com"
IMAP_PORT = 993


def decode_mime_words(s):
    if not s:
        return ""
    decoded = ""
    for part, encoding in decode_header(s):
        if isinstance(part, bytes):
            try:
                decoded += part.decode(encoding if encoding else 'utf-8', errors='ignore')
            except:
                decoded += part.decode('utf-8', errors='ignore')
        else:
            decoded += part
    return decoded


def check_banks_detail():
    print("连接邮箱...")
    mail = imaplib.IMAP4_SSL(IMAP_SERVER, IMAP_PORT)
    mail.login(EMAIL_ADDRESS, PASSWORD)
    print("登录成功！\n")
    
    mail.select('INBOX')
    status, messages = mail.search(None, 'ALL')
    
    email_ids = messages[0].split()
    
    target_keywords = ['建设银行', '民生', '交通银行']
    
    for email_id in reversed(email_ids):
        status, msg_data = mail.fetch(email_id, '(RFC822)')
        
        if status != 'OK':
            continue
        
        for response_part in msg_data:
            if isinstance(response_part, tuple):
                msg = email.message_from_bytes(response_part[1])
                subject = decode_mime_words(msg.get('Subject', ''))
                
                is_target = any(kw in subject for kw in target_keywords)
                
                if is_target:
                    print("="*80)
                    print(f"检查：{subject}")
                    print("="*80)
                    
                    html_body = ""
                    
                    if msg.is_multipart():
                        for part in msg.walk():
                            if part.get_content_type() == 'text/html':
                                try:
                                    charset = part.get_content_charset() or 'utf-8'
                                    if charset.lower() in ['gbk', 'gb2312', 'gb18030']:
                                        html_body = part.get_payload(decode=True).decode('gbk', errors='ignore')
                                    else:
                                        html_body = part.get_payload(decode=True).decode(charset, errors='ignore')
                                except:
                                    pass
                                break
                    
                    full_text = f"{subject}\n{html_body}"
                    
                    print("\n【金额相关信息】")
                    print("-"*80)
                    
                    amount_keywords = [
                        '本期账单金额',
                        '本期应还款',
                        '最低还款额',
                        '账单金额',
                        '应还金额',
                        '本期消费',
                        '交易金额',
                    ]
                    
                    for keyword in amount_keywords:
                        pattern = f'{keyword}.*?￥?([0-9,]+\\.?[0-9]*)'
                        matches = re.findall(pattern, full_text, re.IGNORECASE)
                        if matches:
                            print(f"✓ {keyword}: {matches}")
                    
                    print("\n【还款日信息】")
                    print("-"*80)
                    
                    due_keywords = [
                        '到期还款日',
                        '还款日',
                        '最后还款日',
                        '账单日',
                    ]
                    
                    for keyword in due_keywords:
                        pattern = f'{keyword}.*?([0-9]{{4}}[-/.][0-9]{{1,2}}[-/.][0-9]{{1,2}})'
                        matches = re.findall(pattern, full_text)
                        if matches:
                            print(f"✓ {keyword}: {matches}")
                    
                    print("\n【HTML 关键片段】")
                    print("-"*80)
                    
                    for keyword in ['本期账单金额', '本期应还款', '最低还款额']:
                        pos = full_text.find(keyword)
                        if pos != -1:
                            start = max(0, pos - 100)
                            end = min(len(full_text), pos + 400)
                            print(f"\n包含'{keyword}'的片段：")
                            print(full_text[start:end])
                            print()
                    
                    print("\n" + "="*80)
                    print()
    
    mail.logout()


if __name__ == "__main__":
    check_banks_detail()
